...
def test_json_and_csv_exports_preserve_history_without_tokens(tmp_path):
    ...
    assert len(report["videos"][0]["metric_history"]) == 2
    assert "access_token" not in json_path.read_text(encoding="utf-8")
